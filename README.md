...
**现象**：使用中文输入法（拼音、五笔等）在搜索框输入时，组词过程被中断，拼音字母直接落入搜索框。

**原因**：`renderApp()` 每次 `input` 事件都会用 `innerHTML` 重建整个 DOM（包括搜索框自身），输入法组词状态依附的原 DOM 节点被销毁，组词上下文丢失。`compositionstart`/`compositionend` 在不同浏览器上的事件时序不一致（Safari 的 `input` 可能先于 `compositionstart` 触发），无法可靠拦截。

**最终方案**：对搜索框的 `input` 事件使用 **200ms debounce（防抖）**，不在每次按键时立即重建 DOM：
...
- 输入法组词期间 `input` 事件频繁触发，debounce 定时器被不断重置
- 只有用户停止输入 200ms 后（通常组词已完成），才会重建 DOM
- 筛选下拉框的 `change` 事件不需要 debounce，直接调用 `renderApp()`

**尝试过但无效的方案**：
- 单独使用 `compositionstart`/`compositionend` 跳过渲染 → Safari 事件时序不同，失效
- `renderApp(true)` 用 `outerHTML` 局部替换 → 浏览器对焦点和 IME 状态的处理不一致，仍会打断组词
